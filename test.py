...
first_user_role = None
second_user_name = None
second_user_role = None
log_in = False

class role_selection(customtkinter.CTk):

    def __init__(self, is_second_user=False):
        super().__init__()
        self.geometry("800x600")
        self.is_second_user = is_second_user
        self.title("Eagle Defender")

        self.label = customtkinter.CTkLabel(self, text="Selecciona tu rol")
        self.label.place(relx=0.5, rely=0.1, anchor="center")

        if not self.is_second_user:
            self.attacker_button = customtkinter.CTkButton(self, text="Atacante", command=lambda: self.set_role("Atacante"))
            self.attacker_button.place(relx=0.3, rely=0.7, anchor="center")

            self.defender_button = customtkinter.CTkButton(self, text="Defensor", command=lambda: self.set_role("Defensor"))
            self.defender_button.place(relx=0.7, rely=0.7, anchor="center")
        else:
            role_to_display = "Defensor" if first_user_role == "Atacante" else "Atacante"
            self.button = customtkinter.CTkButton(self, text=role_to_display, command=lambda: self.set_role(role_to_display))
            self.button.place(relx=0.5, rely=0.7, anchor="center")  

    def set_role(self, role):
        global first_user_name
        global first_user_role
        global second_user_name
        global second_user_role
        global log_in
        ...
        if not self.is_second_user:
            ...
            self.destroy()
            app = LogIn_Screen_2()
            app.title("Eagle Defender - Jugador 2")
            app.minsize(800, 600)
            app.mainloop()
        else:
            second_user_role = "Defensor" if first_user_role == "Atacante" else "Atacante"
            self.destroy()
            log_in = True
            app = main_Screen()
            app.title("Eagle Defender")
...
class main_Screen(customtkinter.CTk):
    ...
    def play(self):
        ...
        if log_in == True:
            ...
            print("Inicia sesión para jugar")
...
class LogIn_Screen(customtkinter.CTk):
    ...
    def login(self):
        ...
        if self.entry_username.get() == "admin" and self.entry_password.get() == "123":
            ...
            if user:
                first_user_name = self.entry_username.get()
                self.destroy()
                role_window = role_selection()
...
class LogIn_Screen_2(customtkinter.CTk):
    ...
    def login(self):
        ...
        if user:
            if first_user_name and user[1] == first_user_name:  # Compara usando los nombres de usuario en lugar de los ID, ya que no estás almacenando IDs
                print("No puedes iniciar sesión con el mismo usuario")
            else:
                if first_user_name:
                    print("Ambos usuarios han iniciado sesión exitosamente.")
                    second_user_name = self.entry_username.get()
                    self.destroy()
                    role_window = role_selection(is_second_user=True)
                    role_window.mainloop()
                else:
                    print("Solo el segundo usuario ha iniciado sesión.")
                    first_user_name = None  # Restablecer el primer usuario
        else:
            print("Error en las credenciales")
